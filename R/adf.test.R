#'
#' Calculates ADF Stationarity Test
#'
#' Calculates ADF Test Info based from the urca package
#'
#'@param y Data series
#'@param type The type of test. Whether it includes a drift or trend component.
#'@param lags Number of lags to test
#'@param selectlags Whether a model criterion should be used to determine a lag length.
#'
#'@author Nicholas Dregne and Helena Ristov
#'
#'@export

adf.test <- function (y, type = c("none", "drift", "trend"), lags = 1, selectlags = c("Fixed", "AIC", "BIC")) 
{
  
  if (ncol(as.matrix(y)) > 1) stop("\ny is not a vector or univariate time series.\n")
  if (any(is.na(y)))          stop("\nNAs in y.\n")
